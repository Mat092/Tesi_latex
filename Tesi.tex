...
\begin{figure}[H]
 \centering
 \includegraphics[scale = 0.35]{images/circles-rnd-acc/1}
 \includegraphics[scale = 0.35]{images/circles-rnd-acc/2}
 \includegraphics[scale = 0.35]{images/circles-rnd-acc/3}
...
\section{Profondità della rete e connessioni} 

Due dei parametri importanti da considerare nella costruzione di una rete sono la sua profondità ed il numero di connessioni interne che produce, che ci danno una misura di quanta "informazione" può contenere.
In questo algoritmo non è stata impostata alcuna penalità per individui con troppi layer o troppi \textit{link}, per cui esiste la possibilità che non converga mai ad un risultato finale, costruendo reti sempre più profonde e precise nel separare i dati.
...
Come mostrato nelle figure \ref{moons}, \ref{circles} e \ref{circles+} del capitolo precedente, è chiaro che i dati con noise nullo siano più semplici da separare di quelli con noise diverso da zero.

L'obiettivo è quindi quello di studiare l'andamento di questi parametri in funzione del noise dei dataset.

Inziando dal dataset 
